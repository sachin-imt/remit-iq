...
export default function LiveRateEmbed({ providerA, providerB }: { providerA: string, providerB: string }) {
    ...
    if (loading) {
        return (
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 my-8 flex items-center justify-center h-32">
    ...
    return (
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 my-8 not-prose">
            <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-slate-900 font-bold text-lg m-0">Live Rate Comparison</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                    ...
                    <div key={p.id} className={`p-4 rounded-xl border ${p.id === winner.id ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-slate-200 bg-white'}`}>
                        <div className="flex items-center justify-between mb-3">
                            ...
                            {p.id === winner.id && <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-500 text-slate-900">BEST DEAL</span>}
                        </div>

                        <div className="space-y-1 mb-4">
                            <div className="flex justify-between text-sm">
                                <span className="text-slate-500">Exchange Rate</span>
                                <span className="text-slate-900 font-semibold">₹{p.rate.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                            ...
                            <div className="flex justify-between text-sm pt-2 border-t border-slate-200/50">
                                ...
                                <span className="text-slate-900 font-bold text-lg">₹{formatINR(recv)}</span>
                            </div>
                        </div>
