    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className={`${sig.bg} border ${sig.border} rounded-2xl p-6 flex flex-col justify-center text-center`}>
          ...
          <p className="text-slate-700 text-sm mt-1">{recommendation.reason}</p>
          <div className="mt-3"><p className="text-slate-500 text-xs">Confidence</p>
            <div className="w-full bg-white rounded-full h-2 mt-1"><div className={`h-2 rounded-full ${recommendation.signal === "SEND_NOW" ? "bg-emerald-400" : recommendation.signal === "WAIT" ? "bg-amber-400" : "bg-red-400"}`} style={{ width: `${recommendation.confidence}%` }} /></div>
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
              ...
              <YAxis domain={[stats.low30d - 0.3, stats.high30d + 0.3]} tick={{ fill: "#64748B", fontSize: 11 }} axisLine={false} tickLine={false} width={50} tickFormatter={(v: number) => `₹${v.toFixed(1)}`} />
              <Tooltip contentStyle={{ background: "#FFFFFF", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", border: '1px solid #E2E8F0', borderRadius: 8 }} labelStyle={{ color: '#64748B' }} formatter={(value: number) => [`₹${value}`, "Rate"]} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={s.label} className="bg-slate-50 border border-slate-200 rounded-xl p-4">
            <p className="text-slate-500 text-xs">{s.label}</p><p className="text-slate-900 font-bold text-xl mt-1">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 mb-8">
        <div className="flex items-center gap-2 mb-4"><TrendingUp className="w-5 h-5 text-[#F0B429]" /><h2 className="text-slate-900 font-bold">Rate Intelligence Analysis</h2></div>
        <p className="text-slate-700 mb-4">{recommendation.details}</p>
        <div className="space-y-2 mb-4">
          {recommendation.factors.slice(0, 5).map((f, i) => (
            <div key={i} className="flex items-center gap-3 bg-white rounded-lg p-3">
              ...
              <div className="flex-1"><p className="text-slate-900 text-sm font-semibold">{f.name}</p><p className="text-slate-500 text-xs">{f.description}</p></div>
            </div>
          ))}
        </div>
