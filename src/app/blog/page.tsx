      ...
      <section className="px-4 pt-16 pb-12 md:pt-24 md:pb-16 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-0 max-w-2xl">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-700 leading-tight mb-4">
      ...
      <div className="mx-auto max-w-6xl px-4 pb-24">
        ...
        {featuredPosts.length > 0 && (
          <section className="mb-16">
            <div className="flex items-center gap-2 mb-8 border-b border-slate-200/50 pb-4">
              <TrendingUp className="text-[#F0B429] w-6 h-6" />
              <h2 className="text-2xl font-bold text-slate-900 tracking-wide">Featured Briefing</h2>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {featuredPosts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="group block h-full">
                  <article className="h-full flex flex-col p-8 rounded-2xl border-2 border-slate-200 bg-slate-50 transition-all duration-300 hover:border-[#F0B429]/50 hover:bg-slate-50/80 hover:shadow-2xl hover:shadow-[#F0B429]/5">
                    ...
                    <h3 className="text-2xl md:text-3xl font-extrabold text-slate-700 mb-4 leading-snug group-hover:text-slate-900 transition-colors">
                      {post.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-slate-200/50 mt-auto">
                      ...
                      <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-[#F0B429] transition-colors">
                        <ArrowRight size={18} className="text-[#F0B429] group-hover:text-slate-900 transition-colors" />
                      </div>
                    </div>
                  </article>
                </Link>
        ...
        <section>
          <div className="flex items-center justify-between mb-8 border-b border-slate-200/50 pb-4">
            <h2 className="text-2xl font-bold text-slate-900 tracking-wide">All Articles</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {allPosts.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group block h-full">
                <article className="h-full flex flex-col p-6 rounded-2xl border border-slate-200 bg-slate-50/50 transition-all duration-300 hover:border-[#00B9FF]/50 hover:bg-slate-50 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#00B9FF]/5">
                  <h3 className="text-xl font-bold text-slate-700 mb-3 leading-tight group-hover:text-slate-900 transition-colors line-clamp-2">
                    {post.title}
                  </h3>
        ...
        <section className="mt-16 pt-16 border-t border-slate-200/30">
          <div className="bg-gradient-to-br from-white/80 to-slate-50 border border-slate-200 rounded-3xl p-8 md:p-12 text-center relative overflow-hidden shadow-2xl">
            ...
            <Link href="/">
              <button className="bg-[#F0B429] hover:bg-yellow-400 text-slate-900 font-bold py-3 px-8 rounded-xl transition-all shadow-md hover:shadow-lg hover:-translate-y-1 glow-gold transition-all duration-300">
                Compare Live Rates
              </button>
            </Link>
          </div>
