    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="bg-slate-50 border border-slate-200 rounded-2xl p-8">
        ...
        <div className="mb-6">
          ...
          <div className="relative">
            ...
            <input type="number" step="0.01" min="55" max="75" value={targetRate} onChange={(e) => setTargetRate(e.target.value)}
              className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 pl-10 text-slate-900 text-lg font-bold focus:outline-none focus:border-brand-accent focus:ring-2 focus:ring-brand-accent/20" />
          </div>
        </div>
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {([
              ...
              <label key={opt.value} className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-colors ${alertType === opt.value ? "border-[#F0B429] bg-[#F0B429]/5" : "border-slate-200 hover:border-slate-200/80"}`}>
                ...
                <div><p className="text-slate-900 font-semibold text-sm">{opt.label}</p><p className="text-slate-500 text-xs">{opt.desc}</p></div>
              </label>
            ))}
          </div>
        </div>
        ...
        <button type="submit" disabled={loading}
          className="w-full bg-[#F0B429] text-slate-900 font-bold py-4 rounded-xl hover:bg-yellow-400 transition-colors glow-gold text-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
          {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Setting alert...</> : "Set My Free Alert"}
        </button>
        <p className="text-slate-500 text-xs text-center mt-4">Free forever. No spam. Unsubscribe anytime.</p>
        <div className="mt-4 pt-4 border-t border-slate-200 text-center">
          <a href="/alerts/manage" className="text-sm text-slate-500 hover:text-slate-900 transition-colors underline underline-offset-2">
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={item.title} className="bg-white rounded-xl p-5"><h3 className="text-slate-900 font-semibold text-sm mb-2">{item.title}</h3><p className="text-slate-500 text-xs">{item.desc}</p></div>
        ))}
      </div>
    </div>
  );
